...
def BM_reports_1610_API_pull(start_date, end_date):
    ...
    offshore_installations = {"HMGTO": [219, ["-1", "-2"], "Humber Gateway"],
                              "WTMSO": [200, ["-1"], "Westenmost Rough"]}  # Can't find start of data
    ...
    offshore_installations = {"WDNSO": [500, ["-1", "-2"], "WDNSW"],
                              ...
                              "SHBA": [1000, ["-1", "-2"], "SHBA"]}
    offshore_installations = {"GANW": [353, ["-11", "-13", "-22", "-24"], "Galloper"]}
    offshore_installations = {"LNCSO": [270, ["-1", "-2"], "Lincs"]}
    date = start_date
    #############################
    ...
    plt.show()
    plt.show()
def BM_report_1610_cleaner(raw_hh_data):
    pass
    # find rows where it's all Nan: this implies data outage as opposed to physical outage.

    # Backfill these n rows using data from preceding n rows

    # Sum columns to get total output

    # Divide total output by total capacity at that time, to get load factor
