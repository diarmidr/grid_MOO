...
import pandas as pd
import math
def dispatch_UKES_2024(t_data, x, trunk_dict, capex_dict, ess_dict, ocgt_dict, beccs_dict, graph):
    timestep = 1  # Hours
    ...
    annualised_capex = (trunk_dict["P_Nuclear"] * capex_dict["Nuclear"][0]
                        + trunk_dict["BECCS"] * capex_dict["BECCS"][0]
                        + x[0] * capex_dict["PV"][0]
                        + x[1] * capex_dict["Onshore_wind"][0]
                        + x[2] * capex_dict["Offshore_wind"][0]
                        + x[3] * capex_dict["HES_type"][0]
                        + x[3] * x[4] * capex_dict["HES_type"][1]
                        + x[5] * capex_dict["OCGT"][0]
                        ) * 1e6
    ...
    power_balance_hourly = t_data["nuclear_norm"] * trunk_dict["P_Nuclear"] \
        + t_data["PV_norm"] * x[0] + t_data["offshore_wind_norm"] * x[1] + t_data["onshore_wind_norm"] * x[2] \
        + trunk_dict["BECCS"] - t_data["demand"]/1000
    ...
    for t in range(len(power_balance_hourly)):
        ...
        if power_balance_hourly[t] > 0:
            ...
            for i in range(len(ess_dict)):  # For each ESS in the mix
                ...
                E_ESS = x[i+3] * x[i+3+len(ess_dict)]  # P * duration gives energy capacity.
            ...
            if p_def < 0:
                p_ocgt = min(-p_def, x[5])  # Whichever is lesser of OCGT capacity and deficit
                p_def = p_def + p_ocgt
                p_ocgt_log += [p_ocgt]
            else:
                p_ocgt_log += [0]
    ...
    beccs_cost = trunk_dict["BECCS"] * 1000 * len(t_data) * timestep * beccs_dict["£_per_MWh"]  # MW * h * £/MWh = £
    ...
    beccs_emissions = trunk_dict["BECCS"] * 1000 * len(t_data) * timestep * beccs_dict["kg_CO2_per_MWh"]  # MW * h * kgCO2/MWh = kgCO2
